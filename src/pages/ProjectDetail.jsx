...
function ProjectDetail() {
    ...
    return <div className="project-detail-page">
        <section className="project-hero section"><div className="container"><Link to="/projects" className="back-link"><span className="material-symbols-outlined">arrow_back</span>cd ../projects</Link><div className="project-header"><div className="project-meta"><span className="project-id">PROJECT_ID: {project.id}</span><span className="project-status primary">STATUS: {project.status}</span></div><h1 className="project-title">{project.title}</h1><p className="project-description">{card.description}</p><div className="project-tags">{project.tags.map(tag => <span key={tag} className="tag">#{tag}</span>)}</div></div></div></section>
